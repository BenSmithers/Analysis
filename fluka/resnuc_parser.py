...
all_files = glob( "/home/benito/software/fluka/runs/xenon-136/"+run+"/*_fort*" )
print("Loaded {} files; {} run.".format(len(all_files), run))
...
ax.bar(x=positions, height=values, width=0.8, yerr=errors, align='edge', ecolor='black',capsize=10, label=run)
ax.set_xticks( np.array(positions)+0.4 )
ax.set_xticklabels( labels, size=14 )
ax.set_title("10eV", size=16)
ax.set_ylabel("Isotopes per Neutron", size=16)
ax.set_yscale('log')
ax.text( -0.1, max(values)*0.9 , "FLUKA Preliminary", fontsize=16, color=(1.0,0,0))
plt.tight_layout()
plt.show()
