...
double flux_function(double enu, double cz){
    // enu is in GeV 
    double overall = pow(10, -18);
    double index = -2.;
    double turning = pow(10, 5);

    return overall*pow( (enu/units.GeV)/turning, index );
}
...
int main()
{
  ...
  nus_atm.WriteStateHDF5("./fluxes_flavor.hdf5");
